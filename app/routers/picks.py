...
# ---------------- Slate (timezone-aware daily schedule) ----------------
@router.get(
    "/slate",
    summary="Daily slate by local date",
    description=(
        "Return fixtures/games for a given local calendar date.\n"
        "Handles UTC rollovers by querying [date, date+1) and filtering by the requested timezone.\n\n"
        "Params:\n"
        "- `league`: nba | nfl | ncaaf | ncaab | soccer\n"
        "- `date`: YYYY-MM-DD (interpreted in `tz`)\n"
        "- `tz`: IANA timezone (default America/New_York)\n"
        "- `season`: optional (recommended for NCAAF/NFL)\n"
        "- `league_id_override`: for soccer (competition id, e.g., EPL=39)\n"
    ),
)
def slate(
    league: League,
    date: str,
    tz: str = Query("America/New_York", description="IANA timezone, e.g. America/New_York"),
    season: Optional[int] = None,
    league_id_override: Optional[int] = None,
):
    """
    Fetch a local-date slate robust to UTC rollover.
    """
    settings = get_settings()
    if not settings.apisports_key:
        raise HTTPException(status_code=500, detail="APISPORTS_KEY missing")

    # Parse requested local date
    try:
        req_local = datetime.strptime(date, "%Y-%m-%d").date()
    except ValueError:
        raise HTTPException(status_code=422, detail="date must be YYYY-MM-DD")

    # If tz is invalid and ZoneInfo unavailable, fall back to UTC filtering
    zone = None
    if ZoneInfo:
        try:
            zone = ZoneInfo(tz)
    ...
    client = _client()
    try:
        # Query a 24h UTC span to capture local evening games that rollover to next UTC day
        from_date = date
        to_date = (datetime.strptime(date, "%Y-%m-%d") + timedelta(days=1)).strftime("%Y-%m-%d")

        raw = client.fixtures_range(
            league,
            from_date=from_date,
            to_date=to_date,
            season=season,
        ...
        for g in items:
            # Extract UTC datetime string per sport
            if league == "soccer":
                # API-Football v3
                dt_iso = (g.get("fixture") or {}).get("date")
                home = (g.get("teams") or {}).get("home", {}).get("name")
                away = (g.get("teams") or {}).get("away", {}).get("name")
                fid = (g.get("fixture") or {}).get("id")
                venue_city = (g.get("fixture") or {}).get("venue", {}).get("city")
            else:
                # american-football / basketball v1 typical shapes
                dt_iso = g.get("date") or (g.get("game") or {}).get("date")
            ...
            # Convert to requested local date for filtering
            # Provider sends ISO-8601 with 'Z' or +00:00
            try:
                # Normalize to aware UTC dt
                dt_utc = datetime.fromisoformat(dt_iso.replace("Z", "+00:00"))
                if dt_utc.tzinfo is None:
                    dt_utc = dt_utc.replace(tzinfo=timezone.utc)
            except Exception:
                # If missing/invalid, include conservatively (rare)
                dt_utc = None

            if zone and dt_utc:
                local_date = dt_utc.astimezone(zone).date()
            elif dt_utc:
                local_date = dt_utc.date()  # fallback UTC date
            else:
                # include but mark unknown datetime
                local_date = req_local

            if local_date != req_local:
                continue

            # Compose minimal slate row
            slate_items.append({
                "fixture_id": fid,
                "date": {
                    "timezone": tz if zone else "UTC",
                    "date": date,
                    "time": dt_utc.astimezone(zone).strftime("%H:%M") if (zone and dt_utc) else None,
